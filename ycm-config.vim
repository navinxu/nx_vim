"" YCM configurations
    ...
    " 也就是错误或者警告。只对 C 家族的语言有效。 " 默认为 30 let g:ycm_max_diagnostics_to_display = 20 " 绑定快捷键从而选择第一个自动完成选项。 " 该快捷键可以无限重复循环选择候选项。 " 默认为['<TAB>', '<Down>']。有人用 <Enter>。 let g:ycm_key_list_select_completion = ['<TAB>', '<Down>'] " 选择上一个候选项。 " 默认为：['<S-TAB>', '<Up>']，S 指 Shift 键。 let g:ycm_key_list_previous_completion = ['<S-TAB>', '<Up>'] " 让候选项列表消失。 " 默认为：['<C-y>'] let g:ycm_key_list_stop_completion = ['<C-y>'] " 使用绑定的快捷键触发激活语义完成列表， " 就是 ., ->, :: 等等语义触发条件。 " 默认值为：<C-Space>。 " 若设为空字符串，那么确保没有创建快捷键。
