...
1. 安装 Git。
1. 进入当前用户家目录,并克隆主分支到本地的 .vim 目录上

    ```bash
    git clone https://github.com/navinxu/nx_vim.git .vim
    ```

1. 进入 .vim 目录，并新建一个空目录，名为“bundles”。
1. 在当前用户家目录下创建一个指向本目录下 vimrc　文件的软链接 .vimrc。
1. 做好准备工作。
1. 运行 Vim。
1. 普通模式下输入 :PlugInstall　，按下回车键，稍等。）
...
1. gcc => >= 8.0
1. php
1. flake8 => 支持 Python 的语法检查
1. npm nodejs
1. eslint => 支持 js 的语法检查，通过 npm 来安装
...
1. Ag => 查找文本利器，对应 rking/ag.vim 插件

    ```bash
    sudo apt-get install silversearcher-ag
    ```

1. Ack => 对应 dyng/ctrlsf.vim 插件，全局搜索

    ```bash
    sudo apt-get install -y ack
    ```

1. jdk-8.0 => 以支持 java 语言
...
1. jsonlint => 支持 json 的语法检查

    ```bash
    sudo npm install -g jsonlint
    ```

1. tidy => 支持 HTML 的语法检查

    ```bash
    sudo apt-get install -y tidy
    ```

1. stylelint => 支持 CSS 的语法检查
...
1. pip3 和 powerline => 用于支持 powerline 插件
...
1. vint => 以支持 vim 脚本的检查

    ```bash
    sudo python3 -m pip install vint
    ```

1. shellcheck => 以支持 shell 脚本的检查
...
1. composer => 用于安装 PHP 类库
    安装方法参见：[GetComposer](https://getcomposer.org/)
1. (已废弃)pynvim => deoplete 必须
...
1. flowbin => 支持 JS 插件 autocomplete-flow 需要

    ```bash
    npm install --global flow-bin
    ```

1. ispell => 插件 vimspell 需要

    ```bash
    sudo apt-get install -y ispell
    ```

1. 编译安装 gtags 和　gtags-cscope　以支持　gutentags_plus
